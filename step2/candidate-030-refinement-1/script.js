(() => {
    ...
    function updateStats() {
        elements.tokenBalance.textContent = formatNumber(state.credits);
        elements.spinCost.textContent = formatNumber(CONFIG.spinCost);
        elements.burnRate.textContent = formatNumber(state.totalBurn);
        ...
        if (state.lastNet === null) {
            ...
            elements.lastNet.className = "value neutral";
        } else {
            elements.lastNet.textContent = formatSigned(state.lastNet);
            elements.lastNet.className = `value ${state.lastNet > 0 ? "positive" : state.lastNet < 0 ? "negative" : "neutral"}`;
        }
    ...
    function buildOutcome(symbols, result, net) {
        ...
        if (result.kind === "triple") {
            if (result.payout > 0) {
                return {
                    panelType: "win",
                    stateLabel: "Win",
                    message: `Triple ${result.symbol}. Investor confidence restored.`,
            ...
            return {
                panelType: "loss",
                stateLabel: "Loss",
                message: `Triple ${result.symbol}. Beautifully aligned with failure.`,
                detail: `Payout +0 credits · Net ${formatSigned(net)}.`,
                log: `Inference complete. Triple ${result.symbol} destroyed expected value.`,
                rolledText
            };
        }
        ...
        if (result.kind === "pair") {
            return {
                ...
                message: `Two ${result.symbol}s. A pity payout has been approved.`,
                detail: `Payout +${formatNumber(result.payout)} credits · Net ${formatSigned(net)}.`,
        ...
        return {
            panelType: "loss",
            stateLabel: "Loss",
            message: "No match. Your runway has been converted into vibes.",
            detail: `Payout +0 credits · Net ${formatSigned(net)}.`,
    ...
    async function spin() {
        ...
        if (state.credits <= 0) {
            ...
            setResultPanel(
                "loss",
                "Game Over",
                "You are out of credits. Pivot denied.",
    ...
    function init() {
        ...
        bindEvents();
        updateStats();
        setStatus("Awaiting prompt input...");
        setResultPanel(
            "neutral",
            "Ready",
            "Awaiting prompt input...",
